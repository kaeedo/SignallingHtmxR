...
(function () {
    ...
    onEvent: function (name, evt) {
      switch (name) {
        ...
        case "htmx:beforeProcessNode":
          ...
          forEach(
            queryAttributeOnThisOrChildren(parent, signalRConnect),
            function (child) {
              ensureHubConnection(child);
            }
          );
          forEach(
            queryAttributeOnThisOrChildren(parent, signalRSubscribe),
            function (child) {
              ensureSubscription(child);
            }
          );
          forEach(
            queryAttributeOnThisOrChildren(parent, signalRSend),
            function (child) {
              ensureSending(child);
            }
          );
  ...
  function ensureSubscription(elt) {
    ...
    var signalrSubscribeAttribute = api.getAttributeValue(
      elt,
      signalRSubscribe
    );
    ...
    for (let i = 0; i < signalrMethodNames.length; i++) {
      ...
      hubConnection.on(method, function handler(message) {
        ...
        api.withExtensions(elt, function (extension) {
          messageSpec.message = extension.transformResponse(
            messageSpec.message,
            null,
            elt
          );
  ...
  function processHubConnectionSend(hubElt, sendElt) {
    ...
    triggerSpecs.forEach(function (ts) {
      api.addTriggerHandler(sendElt, ts, nodeData, function (elt, evt) {
        ...
        if (
          !api.triggerEvent(sendElt, "htmx:signalr:beforeSend", {
            method: method,
            headers: headers,
        ...
        api.triggerEvent(sendElt, "htmx:signalr:afterSend", {
          method: method,
  ...
  function maybeUnsubscribe(hubElement, subscription, elt, handler) {
    ...
    if (
      api
        .getAttributeValue(elt, signalRSubscribe)
        .split(",")
